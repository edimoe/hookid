...
local function SafeConnect(event, callback, connectionId, description, group)
    if not event then 
...
local function SafeDisconnect(connectionId)
...
local function SafeCancelThread(threadVar, threadId, description)
...
local function SafeSpawnThread(threadId, callback, description)
...
local function CancelThread(threadVar, threadId, description)
    if not threadVar then return true end
...
local function SpawnThread(threadId, callback, description)
    local thread = task.spawn(function()
        local success, err = pcall(callback)
        if not success then
            warn(string.format("[ThreadManager] Thread '%s' (%s) error: %s", threadId, description or "Unknown", tostring(err)))
...
local HookIDConfig = Window.ConfigManager:CreateConfig("hookid")
...
local function WrapLoopToBudget(loopId, loopFunction, priority, cooldown, originalState)
...
local function InitializeConsolidatedEquip()
...
local RPath = CONSTANTS.RemotePath
...
local RF_SellAllItems = GetRemote(RPath, "RF/SellAllItems", CONSTANTS.ReplionDataTimeout)
...
local ENCHANT_STONE_ID = CONSTANTS.EnchantStoneID
...
premium:Divider()
    ...
    local function DisableV3Physics()
        ...
        -- [FIX BUG] Matikan pemaksa state anti-jatuh DULU sebelum cleanup lain
        -- Ini penting agar stateConnection di-disconnect lebih cepat
    ...
    local function FlyPhysicsTo(targetPos)
        ...
        local SPEED = 80 
        local maxIterations = 500 -- Safety limit (sekitar 8 detik)
        local iterations = 0
        
        while AUTO_9_TOTEM_ACTIVE and iterations < maxIterations do
        ...
        -- Jika keluar karena AUTO_9_TOTEM_ACTIVE false, stop velocity
        if not AUTO_9_TOTEM_ACTIVE and bv then
            bv.velocity = Vector3.new(0, 0.1, 0)
        end
    ...
    local function Run9TotemLoop()
        ...
        AUTO_9_TOTEM_THREAD = task.spawn(function()
            ...
            for i, refSpot in ipairs(REF_SPOTS) do
                ...
                -- Cek UUID setiap loop untuk memastikan masih ada stock
                uuid = GetTotemUUID(selectedTotemName)
                if uuid then
                    TOTEM_STATUS_PARAGRAPH:SetDesc(string.format("Spawning #%d...", i))
                    local success = pcall(function() RE_SpawnTotem:FireServer(uuid) end)
                    
                    if success then
                        spawnedCount = spawnedCount + 1
                        -- Equip tool untuk refresh (optional, tidak wajib)
                        pcall(function() RE_EquipToolFromHotbar:FireServer(1) end)
                    else
                        warn(string.format("[9 Totem] Failed to spawn totem #%d", i))
                    end
                else
                    -- Totem habis, cleanup dan exit
                    WindUI:Notify({ Title = "Totem Habis", Content = string.format("Berhasil spawn %d/9 totem", spawnedCount), Duration = 4, Icon = "alert-triangle" })
                    break
                end
            ...
            -- [FIX BUG] Selalu cleanup meskipun loop break di tengah
            if AUTO_9_TOTEM_ACTIVE then
                TOTEM_STATUS_PARAGRAPH:SetDesc("Returning...")
                FlyPhysicsTo(myStartPos)
                task.wait(0.5)
                WindUI:Notify({ Title = "Selesai", Content = string.format("Berhasil spawn %d/9 totem", spawnedCount), Duration = 3, Icon = "check" })
            end
            
            -- [CLEANUP] Selalu jalankan cleanup
